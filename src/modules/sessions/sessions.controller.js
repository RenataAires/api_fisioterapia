...
const sessionsController = {
  ...
  async cancelSession(req, res, next) {
    ...
    try {
      ...
      // 1. Opcional: Apagar o pagamento se for um cancelamento total
      await client.query("DELETE FROM payments WHERE session_id = $1", [id]);

      // 2. Atualiza o status para Cancelada
      await client.query(
  ...
  // 3. EXCLUSÃO COMPLETA DE SESSÃO (DELETE)
  async deleteSession(req, res, next) {
    ...
    try {
      ...
      if (deletedSession.treatment_plan_id) {
        await client.query(
          `UPDATE treatment_plans 
            SET sessions_done = (SELECT COUNT(*) FROM sessions WHERE treatment_plan_id = $1 AND status = 'Realizada')
            WHERE id = $1`,
          [deletedSession.treatment_plan_id],
        );
      }
